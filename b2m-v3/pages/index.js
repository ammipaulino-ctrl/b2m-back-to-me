...
const T = {
  en: {
    ...
    q3l:["MOTHER","FATHER"],
    q3ph:"Age",
    q3cta:"CONTINUE MY STORY",
    q3skip:"I didn't know them",
    q4:"Is there something you always wanted to know about the day you arrived?",
    q4ph:"Ask El Cronista anything…",
    q4cta:"ASK EL CRONISTA",
    ch2head:"A glimpse of what awaits in Chapter II",
    ch2title:"The Cultural World You Were Born Into",
    ch2sub:"Unlock the music, the faces in power, the headlines, and the atmosphere that surrounded your arrival.",
    ...
    fakePayMsg:"Test Mode: Payment flow coming soon.\n\nYou just simulated a purchase intent.",
  ...
  es: {
    ...
    q3l:["MADRE","PADRE"],
    q3ph:"Edad",
    q3cta:"CONTINUAR MI HISTORIA",
    q3skip:"No los conocí",
    q4:"¿Hay algo que siempre quisiste saber sobre el día que llegaste?",
    q4ph:"Pregúntale a El Cronista lo que quieras…",
    q4cta:"PREGUNTARLE A EL CRONISTA",
    ch2head:"Un vistazo a lo que te espera en el Capítulo II",
    ch2title:"El Mundo Cultural en el Que Naciste",
    ch2sub:"Desbloquea la música, los líderes, los titulares y el ambiente que rodearon tu llegada.",
    ...
    fakePayMsg:"Modo de prueba: el flujo de pago estará disponible pronto.\n\nAcabas de simular una intención de compra.",
...
export default function B2M() {
    ...
    try {
      const res = await fetch("/api/story", {
        ...
        body: JSON.stringify({
          ...
          timeOfDay: extra.timeOfDay,
          motherAge: extra.motherAge,
          fatherAge: extra.fatherAge,
          noParents: extra.noParents,
          openQuestion: extra.openQuestion,
          layer,
          lang
        }),
      });
  ...
  async function answerCity(cityVal) {
    ...
    if (story) {
      setLayers(p => ({...p, 2: story}));
      setActiveL(2);
    }
  }

  async function answerTime(timeVal) {
    const story = await callAPI(3, { timeOfDay: timeVal });
    if (story) {
      setLayers(p => ({...p, 3: story}));
      setActiveL(3);
    }
  }
    ...
    <>
      ...
      {lang && screen === "input" && (
        <div className="page">
          ...
          {err && (
            <p style={{ color:"#C97070", fontSize:13, marginBottom:16, fontStyle:"italic" }}>
              {err}
            </p>
          )}

          <button className="btn-outline fu d4" onClick={startStory}>
            <span>{t.recon}</span>
          </button>
        </div>
      )}
        ...
        <div className="fade-in" style={{ paddingBottom:80 }}>
          ...
          <div className="chapter-bar">
            ...
            <div className="chapter-dots">
              {[1,2,3,4,5].map(n => (
                <div
                  key={n}
                  className={`cdot ${n === activeL ? "active" : n < activeL ? "done" : ""}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="stats-row">
            <div className="stat">
              ...
              <span className="stat-l">{t.stats[0]}</span>
            </div>

            <div className="stat">
              <span className="stat-n">{age}</span>
              <span className="stat-l">{t.stats[1]}</span>
            </div>

            <div className="stat">
              <span className={`stat-n ${genDisplay.length > 12 ? "sm" : ""}`}>
                {genDisplay || "—"}
              </span>
              <span className="stat-l">{t.stats[2]}</span>
            </div>
          </div>
          ...
          {!loading && layers[1] && !layers[2] && (
            <CityQuestion t={t} onSubmit={answerCity} />
          )}
            ...
            <div className="q-wrap">
              ...
              <div className="opt-grid">
                {t.q2o.slice(0,4).map(o => (
                  <button key={o} className="opt-btn" onClick={() => answerTime(o.toLowerCase())}>
                    {o}
                  </button>
                ))}
                <button className="opt-btn full" onClick={() => answerTime("unknown")}>
                  {t.q2o[4]}
                </button>
              </div>
            </div>
          ...
          {!loading && layers[4] && !layers[5] && (
            <OpenQuestion t={t} onSubmit={answerQuestion} />
          )}
                ...
                {CH2_ICONS.map((item, i) => {
                  ...
                  return (
                    <div key={item.key} style={{ position:"relative" }}>
                      ...
                      {isLocked && !ch2data && (
                        <div className="ch2-lock-overlay">
                          <span>◈</span>
                        </div>
                      )}
                    </div>
          ...
          {layers[1] && (
            <div className="chapters-wrap">
              ...
              {t.chapters.map((c,i) => (
                <div
                  key={i}
                  className={`ch-card ${c.active ? "active" : ""} ${c.locked ? "locked" : ""}`}
                >
